...
def get_latest_model_version(model_name):
    ...
    if prod_versions:
        best = max(prod_versions, key=lambda v: int(v.version))
        print(f"Found Production model: version {best.version}")
        return best.version
    
    # Fall back to latest version of any stage
    best = max(versions, key=lambda v: int(v.version))
    print(f"No Production model found. Using version {best.version} (stage: {best.current_stage})")
    return best.version
